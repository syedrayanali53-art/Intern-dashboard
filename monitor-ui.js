(() => {
  ...
  let query='', filter='open', watchlist=true, location='all', hideCitizenOnly=preferences.hideCitizenOnly!==false, hideNoSponsor=preferences.hideNoSponsor!==false, limit=8;
  ...
  const roleKeys = r => ['name:'+r.company.toLowerCase()+'\0'+r.title.toLowerCase(),...(r.company_id?['id:'+r.company_id+'\0'+r.title.toLowerCase()]:[])];
  const markedRoles = label => new Set(Object.values(model.jobs).filter(r=>r.sponsorship===label).flatMap(roleKeys));
  const excluded = (r, citizenOnly, noSponsor) => roleKeys(r).some(key=>(hideCitizenOnly&&citizenOnly.has(key))||(hideNoSponsor&&noSponsor.has(key)));
  ...
  function syncFeed(){const citizenOnly=markedRoles('U.S. Citizenship is Required'),noSponsor=markedRoles('Does Not Offer Sponsorship');window.setMonitorFeed(Object.values(model.jobs).filter(j=>j.status==='open'&&!excluded(j,citizenOnly,noSponsor)).map(toFeed));}
  function render(){
    const all=Object.values(model.jobs), citizenOnly=markedRoles('U.S. Citizenship is Required'), noSponsor=markedRoles('Does Not Offer Sponsorship'), sources=Object.values(model.sources).filter(s=>s.enabled!==false);
    const latest=new Map();
    for(const event of model.events) if(event.job_id) latest.set(event.job_id,event);
    const unread=model.events.filter(e=>['new','reopened'].includes(e.kind)&&e.at>(preferences.readThrough||'')&&(!watchlist||e.company_id)&&(!model.jobs[e.job_id]||!excluded(model.jobs[e.job_id],citizenOnly,noSponsor)));
    const ignored=preferences.ignored||{};
    const selected=all.filter(r=>{
      if(watchlist&&!r.company_id)return false;
      if(excluded(r,citizenOnly,noSponsor))return false;
    ...
    for(const role of selected){const key=role.company_id?'id:'+role.company_id:'name:'+role.company.toLowerCase();if(!groups.has(key))groups.set(key,{name:role.company,jobs:[]});groups.get(key).jobs.push(role);}
    ...
    document.getElementById('monitor').innerHTML=`
      ...
      <div class="monitor-summary"><span>${unread.length} unread role updates</span><span>${all.filter(j=>j.status==='open'&&!excluded(j,citizenOnly,noSponsor)).length} open or discovered roles</span><span>${ok} complete sources</span><span>${partial} partial sources</span><span>${pageWatches.length} page watches</span><span>${failed.length+missing.length} need attention</span></div>
      ...
      <div class="monitor-controls">
        ...
        <label><input type="checkbox" id="ms" ${hideNoSponsor?'checked':''}>Hide jobs without sponsorship</label>
        <button class="mini" id="markRead">Mark updates read</button><button class="mini" id="reloadMonitor">Reload results</button>
      </div>
      <p>${selected.length} matching roles across ${groups.size} companies. Alerts start after a source’s first successful check. “Open” means last reported open by a source.</p>
      <div class="monitor-jobs">${[...groups].slice(0,limit).map(([key,group])=>`<details class="monitor-company" data-company="${safe(key)}" ${query||openCompanies.has(key)?'open':''}>
        <summary><span>${safe(group.name)}</span><span>${group.jobs.length} ${group.jobs.length===1?'internship':'internships'}</span></summary>
        <div class="monitor-company-jobs">${group.jobs.map(r=>{
        ...
        return `<article class="monitor-job"><span class="event">${safe(fresh?event.kind:r.priority+' · '+r.status)}</span>
          <h3>${safe(r.title)}</h3><p>${safe(r.locations.slice(0,3).join(' · ')||'Location not supplied')}${r.locations.length>3?' · +'+(r.locations.length-3)+' more':''}</p>
      ...
      }).join('')}</div></details>`).join('')||'<p>No roles match these filters.</p>'}</div>
      ${groups.size>limit?'<button class="mini" id="moreMonitor" style="margin-top:12px">Show more companies</button>':''}
    ...
    document.getElementById('ms').onchange=e=>{hideNoSponsor=e.target.checked;preferences.hideNoSponsor=hideNoSponsor;savePreferences();syncFeed();render();};
    ...
    document.querySelectorAll('.monitor-company').forEach(group=>group.addEventListener('toggle',()=>{if(query)return;if(group.open)openCompanies.add(group.dataset.company);else openCompanies.delete(group.dataset.company);}));
